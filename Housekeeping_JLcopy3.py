...
def HeaterOn():
    global HeaterCtrl
    res = messagebox.askyesno("Heater On", "Are you sure to turn on the heater?")
    if res == True:
...
def HeaterOff():
    global HeaterCtrl
    res = messagebox.askyesno("Heater off", "Are you sure to turn off the heater?")
    if res == True:
...
def popupwin():
    ...
    # Heaters
    Htronbtn = tk.Button(popup, text="ON", command=HeaterOn)
    Htronbtn.grid(row=7, column=1)
    Htroffbtn = tk.Button(popup, text="OFF", command=HeaterOff)
    Htroffbtn.grid(row=7, column=2)
...
#        window.protocol('WM_DELETE_WINDOW', destroy)  # When you close the tkinter window.
       ...
       #cbPG1.set('/dev/ttyUSB0')
        ...
        lblPG2=tk.Label(window,text='Pressure 2 (bottom)')
       ...
       #cbPG2.set('/dev/ttyUSB1')
        ...
        lblHeaterPwr = tk.Label(window,text='Heater [W]')
        lblHeaterPwr.grid(row=2,column=4)
        ...
        txtT1.grid(row=3,column=1)

        lblT2=tk.Label(window,text='Water Out [deg]')
        lblT2.grid(row=3,column=2)
        txtT2=tkinter.Entry(width=7,justify='right')
        txtT2.grid(row=3,column=3)

        lblT3=tk.Label(window,text='Water In [deg]')
        lblT3.grid(row=3,column=4)
        txtT3=tkinter.Entry(width=7,justify='right')
        txtT3.grid(row=3,column=5)

        lblStatus=tk.Label(window,text='Status')
        lblStatus.grid(row=3,column=7)
